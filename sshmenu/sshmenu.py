...
def display_menu_tg(targets):
    ...
    # Determine the longest Group length
#    longest_group = -1
#    for target in targets:
#        length = len(target['group'])
#        if length > longest_group:
#            longest_group = length
...
def display_menu_g(targets):
    ...
    while True:
        ...
        # Print items
        for index, target in enumerate(targets):
            desc = '%2d ' % (index) + list(target.keys())[0]
            if index == selected_target:
                puts(colored.green(' -> ' + desc))
            else:
                puts('    ' + desc)
        ...
        # Check if key is a number
            ...
            target = targets[selected_target]
            display_menu_tg(target[list(target.keys())[0]]) 
            # Check if there is a custom command for this target
#            if 'command' in target.keys():
#                command = target['command']
#            else:
